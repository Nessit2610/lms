...
public interface CommentReadStatusRepository extends JpaRepository<CommentReadStatus, String> {
	...
	@Modifying
	@Transactional
	@Query("DELETE FROM CommentReadStatus crs WHERE crs.comment = :comment")
	void deleteByComment(@Param("comment") Comment comment);

	@Modifying
	@Transactional
	@Query("DELETE FROM CommentReadStatus crs WHERE crs.commentReply = :commentReply")
	void deleteByCommentReply(@Param("commentReply") CommentReply commentReply);
